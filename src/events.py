...
class CombinedEventDetector:
    ...
    def detect_edge_combined_events(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Detects edge-level combined events based on flow and capacity signals.

        Assumes that the dataframe already contains the columns generated by:
        - SimpleEventDetector.detect_flow_events()
        - SimpleEventDetector.detect_capacity_events()
        ...
        if missing_columns:
            raise ValueError(
                f"Missing required columns for edge combined event detection: {missing_columns}. "
                "Run detect_flow_events() and detect_capacity_events() first, then merge the results."
            )
        ...
        # --------------------------------------------------
        # Some capacity values may be negative in the raw data.
        # For usage calculation, we use the absolute capacity magnitude.
        ...
        df["capacity_usage"] = (
            df["abs_flow_value"] /
            (df["abs_capacity_value"] + 1e-9)
        )

        # --------------------------------------------------
        # Flow + capacity combined events
        # --------------------------------------------------

        df["congestion"] = df["capacity_usage"] >= 0.90

        df["high_flow_low_capacity"] = (
            df["high_flow"] &
            df["low_capacity"]
        )

        df["capacity_drop_with_high_flow"] = (
            df["capacity_drop"] &
            df["high_flow"]
        )

        df["constrained_interconnection"] = (
            df["congestion"] |
            df["high_flow_low_capacity"] |
            df["capacity_drop_with_high_flow"]
        )
        ...
        # --------------------------------------------------
        # Combined edge event flag
        # --------------------------------------------------

        edge_combined_event_columns = [
            "congestion",
        ...
        df["has_edge_combined_event"] = (
            df[edge_combined_event_columns].any(axis=1)
        )
